...
GlossItem::GlossItem(TextBit *baselineText, QObject *parent) : QObject(parent)
{
...
GlossItem::GlossItem(TextBit *baselineText, const QList<TextBit> & textForms, const QList<TextBit> & glossForms, QObject *parent) : QObject(parent)
{
    mBaselineText = baselineText;
    ...
    for(int i=0; i<glossForms.count(); i++)
        mTextItems.insert( glossForms.at(i).writingSystem() , glossForms.at(i).text() );
