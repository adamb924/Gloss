...
void MorphologicalAnalysisModel::setLexicalEntry( qlonglong lexicalEntryId )
{
    ...
    q.prepare( "select TextFormId,group_concat( Transcription , ' ' ) as TextForm, group_concat( Gloss , '-' ) as Gloss from "
                      "(select TextFormId,AllomorphId,Allomorph.Form as Transcription,LexicalEntryGloss.Form as Gloss from MorphologicalAnalysisMembers,Allomorph,LexicalEntryGloss "
                      "where TextFormId in "
                      "( select TextFormId from MorphologicalAnalysisMembers where AllomorphId in "
                      "(select _id from Allomorph where  LexicalEntryId=:LexicalEntryId and WritingSystem=:TextFormWS) ) "
                      "and AllomorphId = Allomorph._id and Allomorph.LexicalEntryId = LexicalEntryGloss.LexicalEntryId and "
                      "LexicalEntryGloss.WritingSystem=:GlossWS "
    ...
    q.prepare( "select * from "
            "(select TextFormId,group_concat( Transcription , ' ' ) as TextForm, group_concat( Gloss , '-' ) as Gloss from "
                      "(select TextFormId,AllomorphId,Allomorph.Form as Transcription,LexicalEntryGloss.Form as Gloss from MorphologicalAnalysisMembers,Allomorph,LexicalEntryGloss "
                      "where TextFormId in "
                      "( select TextFormId from MorphologicalAnalysisMembers where AllomorphId in "
                      "(select _id from Allomorph where  LexicalEntryId=:LexicalEntryId and WritingSystem=:TextFormWS) ) "
                      ...
                      "left join "
                      " ( select TextFormId, Form as Focus from Allomorph,MorphologicalAnalysisMembers on Allomorph._id=MorphologicalAnalysisMembers.AllomorphId and LexicalEntryId=:LexicalEntryId ) as Focus on Focus.TextFormId = Concatenation.TextFormId;"
                      );


*/
    /*
    q.prepare( "select Focus, TextForm, Gloss from "
            "(select TextFormId,group_concat( Transcription , ' ' ) as TextForm, group_concat( Gloss , '-' ) as Gloss from "
                      "(select TextFormId,AllomorphId,Allomorph.Form as Transcription,LexicalEntryGloss.Form as Gloss from MorphologicalAnalysisMembers,Allomorph,LexicalEntryGloss "
                      "where TextFormId in "
                      "( select TextFormId from MorphologicalAnalysisMembers where AllomorphId in "
